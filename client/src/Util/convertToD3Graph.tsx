...
export default function (
    ...
    Object.keys(graph).forEach((v) => {
        let color = '#d3d3d3';
        if (cover.vertices.includes(+v)) color = '#3f51b5';
        else if (kernel.pendant.includes(+v)) color = '#0D8050';
        else if (kernel.tops.includes(+v)) color = '#137CBD';
        else if (kernel.isolated.includes(+v)) color = '#D13913';
        ...
        graph[v].forEach((l: number) => {
            ...
            if (
                links.filter(
                    (link) => (link.source === +v && link.target === l) || (link.source === l && link.target === +v)
                ).length === 0
            ) {
